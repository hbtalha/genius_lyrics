...
void main() {
  ...
  test('Testing songs search', timeout: const Timeout(Duration(minutes: 1)), () async {
    ...
    expect(song?.featuredArtists.any((element) => element.name == 'Anna Wise'), equals(true));
    ...
    expect(song?.featuredArtists.any((element) => element.name == 'Lil Wayne'), equals(true));
    expect(song?.lyrics?.isEmpty, equals(false));
  });
  ...
  test('Testing searching songs by lyrics snippet', timeout: const Timeout(Duration(minutes: 1)), () async {
    List<Song>? song =
        (await genius.searchSongsByLyricsSnippet(lyricsSnippet: 'all the memories collected', getFullInfo: true));

    expect(song?.any((element) => element.title?.contains('XXX.') ?? false), equals(true));
  });
}
